...
export default class MinHash {
    ...
    */
    public hash(terms: string[]): Vector {
    ...
    */
    public hashWithBands(terms: string[], bandWidth: number): Vector {
        ...
        return new Vector(fullHash.map((val) => murmurhash.v3(val.join(""))));
    }

    /** Calculate similarity between two MinHash signatures
    * @return Similarity value between 0 and 1
    */
    public similarity(s1: Vector, s2: Vector): number {
